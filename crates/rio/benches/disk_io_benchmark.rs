...
use rustfs_rio::{DiskFile, RuntimeConfig, RuntimeType, Wal, WalConfig, init_runtime, init_runtime_with_config};
use std::io::Write;
use std::sync::Arc;
...
fn bench_concurrent_ops(c: &mut Criterion) {
    ...
    for &concurrency in CONCURRENCY_LEVELS {
        ...
        group.bench_with_input(BenchmarkId::new("rio_concurrent_writes", concurrency), &concurrency, |b, &concurrency| {
            let data = data.clone();
            b.to_async(&rt).iter(|| async {
                let runtime = init_runtime();
                let temp_dir = TempDir::new().unwrap();
...
fn bench_wal_ops(c: &mut Criterion) {
    ...
    for &entry_size in &entry_sizes {
        ...
        // Single entry append
        group.bench_with_input(BenchmarkId::new("single_append", entry_size), &entry_size, |b, &entry_size| {
            let data = Bytes::from(vec![42u8; entry_size]);
            b.to_async(&rt).iter(|| async {
                let runtime = init_runtime();
                let temp_file = NamedTempFile::new().unwrap();
                let wal = Wal::new(temp_file.path(), runtime, WalConfig::default()).await.unwrap();
        ...
        // Batch append
        group.bench_with_input(BenchmarkId::new("batch_append_100", entry_size), &entry_size, |b, &entry_size| {
            let entries: Vec<Bytes> = (0..100).map(|_| Bytes::from(vec![42u8; entry_size])).collect();
