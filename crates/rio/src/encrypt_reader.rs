    ...
    R: Reader,
{
    ...
    fn batch_encrypt_block(&mut self, data: &[u8]) -> Result<Vec<u8>, std::io::Error> {
        ...
        #[cfg(feature = "metrics")]
        {
            ...
            histogram!("rustfs_encrypt_reader_encryption_duration_seconds")
    ...
    fn encrypt_vectored_io_uring(&mut self, data: &[u8]) -> Result<Vec<u8>, aes_gcm::Error> {
        ...
        self.block_counter = self.block_counter.wrapping_add(
            (data.len() + VECTORED_CHUNK_SIZE - 1) / VECTORED_CHUNK_SIZE as u64
        );
    ...
    async fn test_encrypt_reader_different_block_sizes() {
        ...
        for block_size in [4096, 16384, 65536] {
            ...
            println!("Block size {}: {} -> {} bytes", 
                     block_size, data.len(), result.len());
