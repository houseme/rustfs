...
use rustfs_rio::{
    ...
    EncryptReader, HttpReader, Wal, WalConfig, WarpReader, get_io_engine
};
...
#[cfg(feature = "metrics")]
use rustfs_rio::{IoEngineStats, HttpDownloadStats};
...
async fn main() -> Result<()> {
    ...
    info!("Runtime initialized: {:?} (zero_copy: {})", 
          runtime.runtime_type(), runtime.supports_zero_copy());
...
async fn demo_etag_processing() -> Result<()> {
    ...
    info!("ETag processing: {} bytes -> ETag: {} (took {:?})", 
          result.len(), etag, duration);
...
async fn demo_compression_pipeline() -> Result<()> {
    ...
    for algorithm in [CompressionAlgorithm::Deflate, CompressionAlgorithm::Gzip] {
        ...
        info!("Compression {:?}: {} -> {} bytes (ratio: {:.2}, took {:?})",
              algorithm, data.len(), compressed_data.len(), compression_ratio, duration);
        info!("Compression throughput: {:.2} MB/s",
...
async fn demo_encryption_pipeline() -> Result<()> {
    ...
    info!("Encryption: {} -> {} bytes (overhead: {:.1}%, took {:?})",
          data.len(), encrypted_data.len(), overhead * 100.0, duration);
    info!("Encryption throughput: {:.2} MB/s",
          (data.len() as f64 / (1024.0 * 1024.0)) / duration.as_secs_f64());
...
async fn demo_combined_pipeline() -> Result<()> {
    ...
    info!("Combined pipeline: {} -> {} bytes, ETag: {} (took {:?})",
          data.len(), final_data.len(), etag, duration);
    info!("Pipeline throughput: {:.2} MB/s",
          (data.len() as f64 / (1024.0 * 1024.0)) / duration.as_secs_f64());
...
async fn demo_concurrent_operations() -> Result<()> {
    ...
    let tasks = (0..concurrent_operations).map(|i| {
    ...
    info!("Concurrent operations: {} tasks, {} total bytes (took {:?})",
          concurrent_operations, total_bytes, duration);
    info!("Concurrent throughput: {:.2} MB/s",
...
async fn demo_wal_operations() -> Result<()> {
    ...
    for batch in entries.chunks(batch_size) {
        ...
        info!("Appended batch of {} entries, sequences: {:?}-{:?}",
              batch.len(), sequences.first(), sequences.last());
    }
