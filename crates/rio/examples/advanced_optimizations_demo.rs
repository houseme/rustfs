...
use rustfs_rio::{
    AdvancedRioConfig, MemoryStrategy, PerformanceMonitorConfig, PredictiveOptimizerConfig, init_runtime_with_advanced_config,
};
        ...
        if !trends.is_empty() {
            ...
            for (metric, trend) in trends {
                info!(
                    ...
                    metric, trend.trend_direction, trend.trend_strength, trend.prediction_confidence
                );
            }
        }
    }
...
async fn demonstrate_predictive_optimization(runtime: &rustfs_rio::EnhancedRioRuntime) -> anyhow::Result<()> {
    ...
    if let Some(optimizer) = runtime.predictive_optimizer() {
        ...
        for i in 0..50 {
            ...
            runtime
                .record_performance("sequential_read", 150.0 + (i as f64), 30_000_000.0)
                .await;
        ...
        for i in 0..30 {
            ...
            runtime
                .record_performance("random_read", 400.0 + (i as f64 * 5.0), 10_000_000.0)
                .await;
        ...
        for burst in 0..3 {
            ...
            for i in 0..10 {
                ...
                runtime
                    .record_performance("burst_write", 200.0 + (i as f64 * 2.0), 25_000_000.0)
                    .await;
        ...
        for (operation, pattern) in patterns {
            info!("    📊 {}: {:?} (confidence: {:.2})", operation, pattern.pattern_type, pattern.confidence);
        }
        ...
        info!(
            "    🔮 Prefetch: distance={}, size={} bytes",
            params.prefetch_distance, params.prefetch_size
        );
        ...
        info!(
            "    🔄 Concurrency: {} ops, queue depth {}",
            params.max_concurrent_ops, params.queue_depth
        );
        ...
        if !recommendations.is_empty() {
            ...
            for (i, rec) in recommendations.iter().take(3).enumerate() {
                info!(
                    "    {}. {:?} (confidence: {:.2}, priority: {}, impact: {:.1}%)",
                    i + 1,
            ...
            if let Some(top_rec) = recommendations.first() {
                ...
                if let Err(e) = optimizer.apply_optimization(top_rec.clone()).await {
                    ...
                    info!(
                        "    📊 Updated parameters: batch_size={}, prefetch_distance={}",
...
async fn show_optimization_recommendations(runtime: &rustfs_rio::EnhancedRioRuntime) -> anyhow::Result<()> {
    ...
    info!(
        "    🤖 ML Optimizations: {}",
        if config.enable_ml_optimizations {
            "✅ Enabled"
        } else {
            "❌ Disabled"
        }
