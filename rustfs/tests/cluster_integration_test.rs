...
async fn test_cluster_initialization() {
    ...
    let mut manager = ClusterManager::initialize(cluster_id, total_nodes, config, None)
        .await
...
async fn test_node_failure_recovery() {
    ...
    let mut manager = ClusterManager::initialize(cluster_id, total_nodes, config, Some(1))
        .await
        .expect("Failed to initialize cluster manager");
